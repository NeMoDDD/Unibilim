...
const Verification = () => {
    ...
    const [timer, setTimer] = useState(60); // Изначальное значение таймера (в секундах)s

    const onSubmit = (pinCode) => {
        dispatch(userVerification(otpTokenNikita, pinCode));
        console.log(1);
                ...
                >
                    <div className={s.verification__container}>
                        <form className={s.verification__form} onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className={s.verification__subtitle}>На номер {phoneNumber} было отправлено
                                SMS-сообщение.
                            </div>
                            ...
                            <div className={s.verification__support}>
                                <button onClick={handleResendCode} disabled={isDisabled}
                                        className={!isDisabled ? s.verification__support_again :
                                            `${s.verification__support_again} ${s.verification__support_again_disabled}`}>
                                    Отправить код снова через {timer} секунд
                                </button>
